...
class HenryClient:
    ...
    def atualizar_data_hora(self, dt=None) -> tuple:
        ...
        cmd = f"01+EH+00+{dt_str}]00/00/00]00/00/00"
        ok, resp = self._enviar(cmd)
        sucesso = ok and ("+EH+000" in resp or "+EH+00" in resp)
        return sucesso, resp
    ...
    def liberar_remoto(self, sentido: str = "Entrada", mensagem: str = "Acesso Liberado", mensagem_linha2: str = "BEM-VINDO", tempo_segundos: int = 5) -> tuple:
        """
        Envia comando de liberação remota (REON).
        Se 'mensagem_linha2' for informada, adiciona a segunda linha de texto no display da catraca.
        """
        codigo = "5" if sentido.lower() in ["entrada", "e"] else "6" if sentido.lower() in ["saida", "s"] else "1"
        sufixo_txt = f"}}{mensagem_linha2}" if mensagem_linha2 else ""
        cmd = f"01+REON+00+{codigo}]{tempo_segundos}]{mensagem}{sufixo_txt}]1"
        ok, resp = self._enviar(cmd)
